...
namespace AAMod.NPCs.Bosses.Zero {
    ...
    public class Taser : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            AAWorld.handleAncientBuff(npc, EnumAncients.Zero);
        }
        ...
        public override void AI()
        {
            ...
            double NewRotation = Math.Atan2(num2, num1) - 1.57;
            if (NewRotation < 0) NewRotation += (2 * Math.PI);
            if (Math.Abs(NewRotation - npc.rotation) > Math.PI) {
            ...
            npc.rotation = MathHelper.Lerp(npc.rotation, (float)NewRotation, 1f / 30f);
            npc.rotation %= (float)(2 * Math.PI);
            if (npc.rotation < 0) npc.rotation += (float)(2 * Math.PI);

            npc.dontTakeDamage = ((Zero)zero.modNPC).Hiding;
        }

        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            Texture2D tex = Main.npcTexture[npc.type];
            Texture2D glowTex = mod.GetTexture("Glowmasks/Taser_Glow");
