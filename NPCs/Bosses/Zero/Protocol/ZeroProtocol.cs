        ...
        public void Teleport(int a = 0)
        {
            ...
            if (a == 0)
            {
                while (!safe)
                {
                    ...
                    double radius = 300 + Main.rand.NextDouble() * 300;
                    npc.position = targetPos + (new Vector2((float)Math.Cos(angle), (float)Math.Sin(angle)) * (float) radius);
                    safe = true;
                }
            }
            else if (a == 1)
