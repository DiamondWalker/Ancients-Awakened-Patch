...
namespace AAMod.NPCs.Bosses.Toad
{
    ...
    public class TruffleToad : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            List<int> shroomList = new List<int>();
            for (int i = 0; i < Main.npc.Length; i++) {
                NPC npc2 = Main.npc[i];
                if (npc2 != null && npc2.active && npc2.life > 0 && npc2.type == mod.NPCType("ShroomGlow") && npc2.Distance(npc.Center) < 1000 && Collision.CanHit(npc2.position, npc2.width, npc2.height, npc.Center, npc.width, npc.height)) {
                    shroomList.Add(i);
                }
            } 
            int[] Shrooms = shroomList.ToArray();
            if (Shrooms != null && Shrooms.Length > 0)
            {
                ...
                if(internalAI[3] ++ > 20)
                {
                    npc.life = Math.Min(npc.lifeMax, npc.life + Shrooms.Length);
                    internalAI[3] = 0;
                }
                AIChangeRate = 120;
