...
namespace AAMod.NPCs.Bosses.Athena.Olympian
{
    ...
    public class AthenaA : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (Main.netMode != 1) {
                if (internalAI[2] == 0 && npc.life < (2 * npc.lifeMax) / 3) {
                    NPC.NewNPC((int)npc.position.X, (int)npc.position.Y, ModContent.NPCType<AthenaLight>());
                    internalAI[2] = 1;
                    npc.netUpdate = true;
                }
                if (internalAI[2] == 1 && npc.life < npc.lifeMax / 3) {
        ...
        private bool AliveCheck(Player player)
        {
            ...
            if (player.dead || !player.active || Vector2.Distance(player.Center, Acropolis) > 1280)
            {
                npc.TargetClosest();
                if (player.dead || !player.active || Vector2.Distance(player.Center, Acropolis) > 1280)
                {
                    ...
                    npc.velocity *= 0.94F;
                    if (Main.netMode != 1 && npc.velocity.Length() < 0.15) {
            ...
            if (Main.netMode != 1 && DespawnCounter > 0) DespawnCounter--;
