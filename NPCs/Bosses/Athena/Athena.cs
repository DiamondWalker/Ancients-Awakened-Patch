    ...
    public class Athena : ModNPC
    {
        ...
        public override void AI()
        {
                    ...
                    if (Main.netMode != 1)
                    {
                        if (!Seen)
                        {
                            ...
                            if (internalAI[4] >= 300)
                            {
                                ...
                                Main.npc[p].direction = npc.direction;
                                npc.active = false;
                            }
                            return;
                        }
            ...
            else
            {
                ...
                if (player.dead || !player.active || !modPlayer.ZoneAcropolis)
                {
                    npc.TargetClosest();
                    if (player.dead || !player.active || !modPlayer.ZoneAcropolis)
                    {
                ...
                if (internalAI[1] == 0) //Acropolis Phase
                {
                    ...
                    if (Vector2.Distance(player.Center, Acropolis) > 1280)
                    {
                        ...
                        if (npc.velocity.Length() < 0.15)
                        {
                            ...
                            {
                                if (Main.netMode != 1) {
                                    BaseUtility.Chat(Lang.BossChat("Athena9"), Color.CornflowerBlue);
                                    int p = NPC.NewNPC((int)npc.position.X, (int)npc.position.Y, ModContent.NPCType<AthenaFlee>());
                                    Main.npc[p].Center = npc.Center;
                                    Main.npc[p].direction = npc.direction;
                                    npc.active = false;
                ...
                else //Cloud Phase
                {
                    ...
                    if (Main.netMode != 1)
                    {
                        if (npc.ai[1] > 150 && (npc.ai[1] == 400 || !NPC.AnyNPCs(ModContent.NPCType<SeraphA>())))
                        {
                            if (npc.ai[3]-- == 0)
                            {
                                internalAI[1] = 0;
                    ...
                    if (Vector2.Distance(npc.Center, MoveVector2) < 10)
                    {
                        ...
                        if (npc.ai[1] == 150 && Main.netMode != 1)
                        {
                            NPC Seraph1 = Main.npc[NPC.NewNPC((int)npc.Center.X + 100, (int)npc.Center.Y, ModContent.NPCType<SeraphA>())];
                            for (int i = 0; i < 3; i++)
                            {
                            ...
                            NPC Seraph2 = Main.npc[NPC.NewNPC((int)npc.Center.X - 100, (int)npc.Center.Y, ModContent.NPCType<SeraphA>())];
                            for (int i = 0; i < 3; i++)
                            {
        ...
        {
            int damage = npc.damage == 0 ? npc.defDamage : npc.damage;
            if (Main.expertMode)
            {
                damage /= 4;
            }
