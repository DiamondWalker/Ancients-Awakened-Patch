...
namespace AAMod.NPCs.Bosses.Anubis
{
    ...
    public class Anubis : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            switch (npc.ai[0])
            {
                case 0:
                    ...
                    BaseAI.ShootPeriodic(npc, player.position, player.width, player.height, proj, ref npc.ai[3], 80, npc.damage / 2, 10, true);
                ...
                case 3:
                    ...
                    if (npc.life > npc.lifeMax * (2/3))
                    {
                        ...
                        if (npc.ai[1] > 120 && !AAGlobalProjectile.AnyProjectiles(ModContent.ProjectileType<Block>()) && !AAGlobalProjectile.AnyProjectiles(ModContent.ProjectileType<Block1>()))
                        {
                            npc.ai[0]++;
                            npc.ai[1] = 0;
                            npc.ai[2] = 0;
                ...
                default:
                    npc.ai[0] = 0;
                    npc.ai[3] = 39; // ensure a warm up period before shooting rune blasts
                    goto case 0;
            }
        }
                ...
                if (!player.active || player.dead || Vector2.Distance(npc.Center, player.Center) > 5000f || !player.ZoneDesert) {
                    if (Main.netMode != 1) {
                        ...
                        if (!player.active || player.dead) {
                            ...
                            if (deathtimer == 200) {
                                if (Main.netMode != 1) BaseUtility.Chat(Lang.BossChat("AnubisWarn"), Color.Gold);
                            } else if (deathtimer > 400) {
                                BaseUtility.Chat(Lang.BossChat("AnubisForfeit"), Color.Gold);
                                npc.active = false;
                            }
                        }
                        if (!npc.active) {
                            BaseUtility.Chat(Lang.BossChat("AnubisFalse"), Color.Gold);
        ...
        public void Teleport()
        {
            ...
            Player player = Main.player[npc.target];
            double angle = (Main.rand.NextDouble() * 4 * Math.PI / 6) + Math.PI / 6;
            Vector2 relativePos = new Vector2((float)Math.Cos(angle), (float)-Math.Sin(angle)) * 300;

            npc.position = player.Center + relativePos;
            int pieCut = 20;
