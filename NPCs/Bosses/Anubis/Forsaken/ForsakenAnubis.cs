...
namespace AAMod.NPCs.Bosses.Anubis.Forsaken
{
    ...
    public class ForsakenAnubis : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            switch (npc.ai[0])
            {
                ...
                case 5:
                    ...
                    if (npc.life > npc.lifeMax / 2)
                    {
                        if (npc.ai[1] > 160 && !AAGlobalProjectile.AnyProjectiles(ModContent.ProjectileType<Block>()) && !AAGlobalProjectile.AnyProjectiles(ModContent.ProjectileType<Block1>()))
                        {
                            internalAI[1]++;
                            if (internalAI[3] < TeleportCount && internalAI[1] >= 50)
                            {
                    ...
                    {
                        if (npc.ai[1] > 240 && !AAGlobalProjectile.AnyProjectiles(ModContent.ProjectileType<BlockF>()) && !AAGlobalProjectile.AnyProjectiles(ModContent.ProjectileType<BlockF1>()))
                        {
                            internalAI[1]++;
                            if (internalAI[3] < TeleportCount && internalAI[1] >= 50)
                            {
                ...
                if (!player.active || player.dead || Vector2.Distance(npc.Center, player.Center) > 5000f || !player.ZoneDesert) {
                    if (Main.netMode != 1) {
                        ...
                        if (!player.active || player.dead) {
                            ...
                            if (deathtimer == 120) {
                                if (Main.netMode != 1) BaseUtility.Chat(Lang.BossChat("FAnubisWarn"), Color.ForestGreen);
                            } else if (deathtimer > 240) {
                                npc.active = false;
                            }
                        }
                        if (!npc.active) {
                            BaseUtility.Chat(Lang.BossChat("FAnubis"), Color.ForestGreen);
        ...
        public void Teleport()
        {
            ...
            Vector2 relativePos = new Vector2((float)Math.Cos(angle), (float)-Math.Sin(angle)) * 500;
