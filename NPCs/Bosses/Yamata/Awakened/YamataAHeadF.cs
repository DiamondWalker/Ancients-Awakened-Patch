    ...
    public class YamataAHeadF : ModNPC
    {
            ...
            if (targetPlayer != null) {
                switch (Body.currentAttack) {
                    ...
                    case 4: //shoot direct aim deathrays
                        float deathrayFireTime = npc.ai[3] * 60 - 30;
                        if (Body.attackTime == deathrayFireTime) {
                            if (Main.netMode != 1)
                        ...
                        if (Body.attackTime >= deathrayFireTime && Body.attackTime <= deathrayFireTime + 180) deathRay = 30;
                        break;
        ...
        public override void FindFrame(int frameHeight)
        {
            if (deathRay > 0) {
                ...
                if (npc.frameCounter < deathRay) {
                    ++npc.frameCounter;
                } else {
                    npc.frame.Y += frameHeight;
                }
            } else {
                if (npc.frameCounter > 10) npc.frameCounter = 10;
                if (npc.frameCounter > 0) {
                    if (npc.frameCounter-- < 5) npc.frame.Y = frameHeight;
                } else {
                    npc.frame.Y = 0;
                }
            }
