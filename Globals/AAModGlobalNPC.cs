...
namespace AAMod
{
    public class AAModGlobalNPC : GlobalNPC
    {
        ...
        public override bool PreAI(NPC npc)
        {
            ...
            if (npc.type == NPCID.MoonLordCore && bossTitle <= 120) {
                if (bossTitle++ == 120) {
                    AAMod.ShowTitle(npc, 19);
                }
            }
        ...
        public override void EditSpawnPool(IDictionary<int, float> pool, NPCSpawnInfo spawnInfo)
        {
            ...
            if (spawnInfo.player.GetModPlayer<AAPlayer>().ZoneInferno)
            {
                ...
                {
                    pool.Add(mod.NPCType("Flamebrute"), .025f);
                    pool.Add(mod.NPCType("InfernoSalamander"), .05f);
                    pool.Add(mod.NPCType("Wyrmling"), .02f);
                    pool.Add(mod.NPCType("InfernalSlime"), .025f);
                    pool.Add(mod.NPCType("DragonClaw"), .025f);

                    if (Main.hardMode)
                    {
                        pool.Add(mod.NPCType("MagmaSwimmer"), SpawnCondition.WaterCritter.Chance * 0.02f);
                        pool.Add(mod.NPCType("BlazePhoenix"), .01f);
                    }

                    if (AAWorld.downedSisters)
                    {
                        pool.Add(mod.NPCType("BlazeClaw"), .025f);
                    }
                }
                else if (spawnInfo.player.position.Y > (Main.worldSurface * 16.0))
                {
                    ...
                    if (Main.hardMode)
                    {
                        pool.Add(mod.NPCType("MagmaSwimmer"), SpawnCondition.WaterCritter.Chance * 0.04f);
                        pool.Add(mod.NPCType("Wyrm"), .008f);
                        pool.Add(mod.NPCType("ChaoticDawn"), .005f);

                        if (spawnInfo.player.ZoneSnow)
                        {
                            pool.Add(mod.NPCType("Dragron"), .005f);
                        }

                        if (spawnInfo.player.ZoneUndergroundDesert)
                        {
                            pool.Add(mod.NPCType("InfernoGhoul"), .025f);
                        }
                    }
            ...
            if (spawnInfo.player.GetModPlayer<AAPlayer>().ZoneMire)
            {
                ...
                if ((spawnInfo.player.position.Y < (Main.worldSurface * 16.0)) && (!Main.dayTime || AAWorld.downedYamata))
                {
                    ...
                    if (!AAWorld.downedSisters && AAWorld.downedHydra && !NPC.AnyNPCs(mod.NPCType("HarukaShadow"))) pool.Add(mod.NPCType("HarukaShadow"), .00005f);
                    ...
                    if (AAWorld.downedSisters)
                    {
                        pool.Add(mod.NPCType("AbyssClaw"), .025f);
                    }
                }
                else if (spawnInfo.player.position.Y > (Main.worldSurface * 16.0))
                {
