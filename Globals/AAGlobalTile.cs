...
namespace AAMod {
    public class AAGlobalTile : GlobalTile
    {
        ...
        public static Color GetIncineriteColor(Color color, float min, float max, bool clamp) => GetTimedColor(AAColor.IncineriteColor, color, min, max, clamp);
        ...
        {
            if (NPC.downedPlantBoss && (Main.tile[i, j].type == TileID.MushroomGrass || Main.tile[i, j].type == (ushort)ModContent.TileType<Mycelium>()))
            {
